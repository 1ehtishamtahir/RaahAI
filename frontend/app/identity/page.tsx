...
function IdentityInner() {
  ...
  const svcParam = searchParams.get("svc") || "cnic";
  const [list, setList] = useState<any>(null);
  const [active, setActive] = useState(svcParam);
  const [flow, setFlow] = useState<any>(null);

  useEffect(() => { identityListApi().then(setList).catch(()=>{}); }, []);
  useEffect(() => { setActive(svcParam); }, [svcParam]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {(list?.services || ["cnic","passport","frc","birth_registration","marriage_registration","death_registration"]).map((svc:string) => {
          ...
          return (
            <button key={svc} onClick={()=>select(svc)} className={`text-left p-4 rounded-2xl border-2 transition ${active===svc?"border-raah-green bg-raah-mint":"border-border bg-white hover:border-raah-green/30"} ${isPrimary?"ring-1 ring-raah-green/20":""}`}>
